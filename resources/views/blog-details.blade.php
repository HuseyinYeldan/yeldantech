...
<body class="rbt-header-sticky">
                ...
                <ul class="meta-list justify-content-center mb--10">
                    <li class="list-item">
                        <div class="author-thumbnail">
                            <img src="{{Voyager::Image($blog->user->avatar)}}" alt="{{$blog->user->name}} kullanıcısının profil resmi">
                        </div>
                        <div class="author-info">
                            paylaşan <a href="/profil/{{$blog->user->name}}"><strong>{{$blog->user->name}}</strong></a>
                            <img src="/assets/images/icons/verified.webp" alt="Onaylı Kullanıcı İşareti" style="width:20px;">
                        </div>
                    </li>
                    <li class="list-item">
                    ...
                    <li class="list-item">
                        <b class="color-primary">⚡Ortalama {{ floor(count(preg_split('/\s+/u', strip_tags($blog->content)))/120) }} dakika </b>
                    </li>
                    <li class="list-item">
                    ...
                    @if(Auth::check() && Auth::user()->id === $blog->user->id ||Auth::check() && Auth::user()->role_id == 1)
                        <a href="/blog-duzenle/{{$blog->id}}" class="custom-add-blog2" style="width:100%; font-size:12pt">Blogu Düzenle <i class="feather-edit"></i> </a>
                    @endif
                </ul>
            ...
            <div class="blog-content-wrapper rbt-article-content-wrapper">
                ...
                <div class="content" id="content">
                        ...
                        <form action="{{ route('comment.share', ['blogId' => $blog->id]) }}" method="POST">
                            ...
                            <div class="row row--10">
                                ...
                                <div class="col-lg-12 mt--10">
                                    <button class="rbt-btn btn-gradient icon-hover radius-round btn-md" type="submit">
                    ...
                    <div class="comment-respond">
                        ...
                        <form action="{{ route('comment.share', ['blogId' => $blog->id]) }}" method="POST">
                            ...
                            <p class="comment-notes"><span id="email-notes">Email adresiniz görünmeyecektir.</span></p>
                            <div class="row row--10">
                                <div class="col-lg-6 col-md-6 col-12">
                                    <div class="form-group">
                                        ...
                                        @endif
                                        <input id="name" type="text" name="name" @if(request()->cookie('comment_name')) placeholder="Ad:" value="{{request()->cookie('comment_name')}}" @endif> <!-- Add name attribute -->
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6 col-12">
                                    <div class="form-group">
                                        ...
                                        @endif
                                        <input id="email" type="email" name="email" @if(request()->cookie('comment_email')) placeholder="Email: " value="{{ request()->cookie('comment_email') }}" @endif > <!-- Add name attribute -->
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="form-group">
                                ...
                                <div class="col-lg-12">
                                    <p class="comment-form-cookies-consent">
                                        <input id="comment-cookies-consent" name="comment-cookies-consent" type="checkbox" checked>
                                        <label for="comment-cookies-consent">Bir dahaki yorumum için bilgilerimi çerezlere kaydet.</label>
                                    </p>
                                </div>
                                <div class="col-lg-12">
                                    <button class="rbt-btn btn-gradient icon-hover radius-round btn-md" type="submit">
                    ...
                    <div class="rbt-comment-area">
                            ...
                            <li class="comment">
                                <div class="comment-body">
                                    <div class="single-comment"  style="position: relative">
                                        @if(auth()->check() && auth()->user()->role_id == 1)
                                        <form method="post" action="{{ route('comments.delete', ['comment' => $comment->id]) }}">
                                            @csrf
                                            @method('DELETE')
                
                                            <button type="submit" class="btn btn-danger" style="font-size:10pt; font-weight:bold; position: absolute; right:0;"> Yorumu Sil <i class="feather-trash" style="font-weight: 500"></i> </button>
                                        </form>
                                        @endif
                                        <div class="comment-img">
                                            ...
                                            @if($comment->user)
                                                ...
                                                <img src="{{Voyager::Image($comment->user->avatar)}}" alt="{{$comment->user->name}} kullanıcısının profili">
                                                </a>
                                            @else
                                                <img src="/assets/images/favicon.png" alt="Anonim Yeldan Tech Profili">
                                            @endif
                                        </div>
                                        <div class="comment-inner">
                                            ...
                                            <h6 class="commenter">
                                                @if($comment->user)
                                                   <b><a href="/profil/{{$comment->user->name}}">{{ $comment->user->name }}</a> </b> 
                                                    @if($comment->user->role_id == 1 || $comment->user->role_id == 3)
                                                        <img src="/assets/images/icons/verified.webp" alt="Onaylı Kullanıcı İşareti" style="width:20px;">
                                                    @endif
                                                @else
                                                    {{ $comment->name }} <sup>(Anonim)</sup> <!-- Assuming 'name' is a field in noauthcomments table -->
                                                @endif
                                            </h6>
                        ...
                        <hr>
                        <br>
                        <b class="title"> Onaylamayı Bekleyen {{$waitingComments->count()}} Yorum Mevcut</b>
                        @foreach ($waitingComments  as $bekleyen)
                        <li class="comment mt--20" id="waitingcomments">
                            <div class="comment-body">
                                <div class="single-comment"  style="position: relative">
                                    @if(auth()->check() && auth()->user()->role_id == 1)
                                    <form method="post" action="{{ route('comments.delete', ['comment' => $bekleyen->id]) }}">
                                        @csrf
                                        @method('DELETE')
            
                                        <button type="submit" class="btn btn-danger" style="font-size:10pt; font-weight:bold; position: absolute; top:60px; right:0;"> Yorumu Sil <i class="feather-trash" style="font-weight: 500"></i> </button>
                                    </form>

                                    <form method="post" action="{{ route('comments.approve', ['comment' => $bekleyen->id]) }}">
                                        @csrf
                                        <button type="submit" class="btn btn-success" style="font-size:10pt; font-weight:bold; position: absolute; top:25px; right:0;"> Yorumu Onayla <i class="feather-check" style="font-weight: 500"></i> </button>
                                    </form>
                                    ...
                                    <div class="comment-img">
                                        ...
                                        @if($bekleyen->user)
                                            ...
                                            <img src="{{Voyager::Image($bekleyen->user->avatar)}}" alt="{{$bekleyen->user->name}} kullanıcısının profili">
                                            </a>
                                        @else
                                            <img src="/assets/images/favicon.png" alt="Anonim Yeldan Tech Profili">
                                        @endif
                                    ...
                                    <div class="comment-inner">
                                        ...
                                        <h6 class="commenter">
                                            @if($bekleyen->user)
                                               <b><a href="/profil/{{$bekleyen->user->name}}">{{ $bekleyen->user->name }}</a> </b> 
                                                @if($bekleyen->user->role_id == 1 || $bekleyen->user->role_id == 3)
                                                    <img src="/assets/images/icons/verified.webp" alt="Onaylı Kullanıcı İşareti" style="width:20px;">
                                                @endif
                                            @else
                                                {{ $bekleyen->name }} <sup>(Anonim)</sup> <!-- Assuming 'name' is a field in noauthcomments table -->
                                            @endif
                                        </h6>
                                        <div class="comment-meta">
                                            <div class="time-spent">{{ $bekleyen->created_at->diffForHumans() }}</div>
                                        </div>
                                        <div class="comment-text">
                ...
                <div class="related-post pt--60">
                    <div class="section-title text-start mb--40">
                        <h4 class="title">Son Bloglarımız</h4>
                    </div>


                    @foreach ($allblogs->slice(0,4)->sortBy('created_at') as $bloglar)
                    <!-- Start Single Card  -->
                    <div class="rbt-card card-list variation-02 rbt-hover mt--30" style="overflow: visible">
                        <div class="rbt-card-img">
                            <a href="/blog/{{$bloglar->slug}}">
                                <img src="{{Voyager::Image($bloglar->image)}}" alt="{{$bloglar->title}} bloğu görseli"> </a>
                        </div>
                        <div class="rbt-card-body">
                            <h5 class="rbt-card-title"><a href="/blog/{{$bloglar->slug}}">{{$bloglar->title}}</a></h5>
                            <a class="mb--20" href="/blog/{{$bloglar->slug}}">{{$bloglar->excerpt}}</a>
                            <div class="rbt-card-bottom">
                                <a class="transparent-button" href="/blog/{{$bloglar->slug}}">Hemen Oku<i><svg width="17" height="12" xmlns="http://www.w3.org/2000/svg"><g stroke="#27374D" fill="none" fill-rule="evenodd"><path d="M10.614 0l5.629 5.629-5.63 5.629"/><path stroke-linecap="square" d="M.663 5.572h14.594"/></g></svg></i></a>
                            </div>
                        </div>
                    </div>
                    <!-- End Single Card  -->
    ...
    <div class="social-share">
        <a href="https://www.facebook.com/sharer/sharer.php?u=http%3A//127.0.0.1%3A8000/blog/{{$blog->slug}}"> <i class="fa-brands fa-square-facebook"></i> </a>
        <a href="https://twitter.com/intent/tweet?text=http%3A//yeldan.tech/blog/{{$blog->slug}}"> <i class="fa-brands fa-square-x-twitter"></i> </a>
